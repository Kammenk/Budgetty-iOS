...
- **Verified:** Debug build clean; 107/107 unit tests pass; Insights legend + Budget render (iPhone 17 Pro, dark). **NOT tap-verified** — the MCP simulator's `xcode-select` preflight is stale-blocked on this Mac (needs `sudo xcode-select -s …`), so slice/row tap-select, emoji-search typing, and the picker context menu are logic-tested only. Same limitation as the period/money-flow port.*
